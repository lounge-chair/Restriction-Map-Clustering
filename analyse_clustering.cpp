...
int main(int argc, char *argv[])
{
    ...
    if(kmer_str.compare("kmerinfo/4merinfo.txt") == 0)          respath += "/4mers";
    else if(kmer_str.compare("kmerinfo/5merinfo.txt") == 0)     respath += "/5mers";
    else if(kmer_str.compare("kmerinfo/6merinfo.txt") == 0)     respath += "/6mers";
    else {
        cout << "Unknown kmerinfo filename! Exiting." << endl;
        return(1);
    }

    if((model_str == "model_predictions/4new_model_predictions_200.csv") || (model_str == "model_predictions/5new_model_predictions_200.csv") || (model_str == "model_predictions/6new_model_predictions_200.csv")) {
        respath += "/clustering_200";
    }          
    else if((model_str == "model_predictions/4new_model_predictions_400.csv") || (model_str == "model_predictions/5new_model_predictions_400.csv") || (model_str == "model_predictions/6new_model_predictions_400.csv")) {
        respath += "/clustering_400"; 
    }
    else if((model_str == "model_predictions/4new_model_predictions_600.csv") || (model_str == "model_predictions/5new_model_predictions_600.csv") || (model_str == "model_predictions/6new_model_predictions_600.csv")) {
        respath += "/clustering_600"; 
    }    
    else if((model_str == "model_predictions/4new_model_predictions_800.csv") || (model_str == "model_predictions/5new_model_predictions_800.csv") || (model_str == "model_predictions/6new_model_predictions_800.csv")) {
        respath += "/clustering_800"; 
    }
    ...
    {                           // get current "Rmap_x" label and store to str
    ...
    for (int dVal = 1; dVal < 16; dVal++)
    {
        for (int LIC = 5000; LIC < 75000; LIC += 5000) 
        {
            cout << "dVal: " << dVal << endl;
            cout << "LIC: "  << LIC  << endl;
            ...
            cout << endl
                 << relation_matrix.size() << " x " << relation_matrix[0].size() << endl;
            ...
            for (int i = 0; i < rmap_to_cluster.size(); i++)
            {
                ...
                for (int j = 0; j < relation_matrix[i].size(); j++)
                {
                    ...
                    if (relation_matrix[i][j] > dVal)
                    { //D VALUE PRESENT HERE
            ...
            /////////////////////////////////
            float precision = (float)totcorrect * 100 / (totcorrect + totfalse);
            float recall = (float)recor * 100 / (recor + refal);
